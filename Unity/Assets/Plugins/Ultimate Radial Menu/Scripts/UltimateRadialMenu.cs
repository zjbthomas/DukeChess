...
public class UltimateRadialMenu : MonoBehaviour
{
	...
	public class UltimateRadialButton
	{
		...
		public void OnEnter ()
		{
			...
			// If the current selected index is assigned and it is the same as this button index, then select this button.
			if( radialMenu.CurrentSelectedButtonIndex < 0 || radialMenu.CurrentSelectedButtonIndex != buttonIndex )
			{
	...
	public void ProcessInput ( Vector2 input, float distance, bool inputDown, bool inputUp )
	{
		...
		for( int i = 0; i < UltimateRadialButtonList.Count; i++ )
		{
			...
			// If there was a current button with input on it, and the input is still within the angle of that button, then assign the loop index to the current button index.
			if( CurrentButtonIndex >= 0 && UltimateRadialButtonList[ CurrentButtonIndex ].IsInAngle( angle ) )
	...
	void RadialMenuButtonCountModified ()
	{
		...
		if( radialMenuStyle != null )
		{
			...
			for( int i = 0; i < UltimateRadialButtonList.Count; i++ )
			{
				...
				else if( CurrentSelectedButtonIndex >= 0 && CurrentSelectedButtonIndex == i )
	...
	public void UpdatePositioning ()
	{
		...
		// If the render mode is set to world space...
	...
	{
		// Assign the radial button of the info to this index.
		...
		// If the user wants an icon for the button...
		if( useButtonIcon )
		{
		...
		// If the user wants to display text for the buttons...
		if( useButtonText )
		{
	...
	{
		// If the button doesn't exist at the index then log a warning and return null.
	...
	public UltimateRadialButton GetUltimateRadialButton ( string buttonName )
	{
		...
		// If the index is less than zero then there is no button with this name, so log a warning and return null.
		if( buttonIndex < 0 )
		{
		...
		// Return the button at the index that has been registered with the targeted name.
		return UltimateRadialButtonList[ buttonIndex ];
	}
	...
	public void UpdateRadialButton ( string buttonName, Action ButtonCallback, UltimateRadialButtonInfo newRadialButtonInfo )
	{
		...
		UltimateRadialButtonList[ buttonIndex ].AddCallback( ButtonCallback );// UPDATED
	}
	...
	{
		// Create a new button at the end of the list.
		CreateRadialButtonAtIndex( 1000 );

		// Register the button information to the new button.
	...
	public static UltimateRadialButton GetUltimateRadialButton ( string radialMenuName, string buttonName )
	{
		...
		// Return the result of getting the targeted Ultimate Radial Button.
